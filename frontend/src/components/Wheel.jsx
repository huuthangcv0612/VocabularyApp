  ...
  const evaluateFeedback = (sentenceText, selectedWord, feedbackData) => {
    ...
    return {
      ...
      wortschatz: {
        ...
        reasons: uniq([
          ...(wortschatzValue !== "passend" ? wordChoiceIssues.map((c) => c.message_vi || c.message) : []),
          !usedWord ? `Câu cần chứa từ: ${selectedWord?.word || "từ được cho"}` : null,
        ]),
      },
      satzbau: {
        label: "Satzbau",
        value: wordOrderIssues.length === 0 ? "korrekt" : "verbessern",
                        ...
                        {feedback.checks.map((c, idx) => (
                          <li key={idx} className="text-slate-700">
                            ...
                            {c.replacements && c.replacements.length > 0 && (
                              <div className="mt-2 ml-2 p-2 bg-blue-50 rounded border border-blue-200">
                                <strong className="text-sm text-slate-600">Câu sửa:</strong>
                                <div className="text-slate-900 font-semibold">{feedback.suggestedSentence}</div>
                              </div>
                            )}
                          </li>
                        ))}
